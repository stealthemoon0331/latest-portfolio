...
type ImageLike = { src: string; alt?: string } | string;

const DEFAULT_IMAGES: Array<ImageLike> = Array.from({ length: 24 }, (_, i) => ({
  src: `/gallery/${i + 1}.jpg`,
...
function buildItems(pool: Array<ImageLike>, seg: number) {
...
export default function DomeGallery({
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity = [0, 0], direction = [0, 0], movement }) => {
        ...
        if (last) {
          ...
          let vx = vMagX * (dirX as number);
          let vy = vMagY * (dirY as number);
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (!originalPos) {
        ...
        (el as HTMLElement).style.zIndex = "0" as any;
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      void (refDiv as any).offsetHeight;
      ...
      (viewerRef.current as HTMLDivElement).appendChild(overlay);
          ...
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => (
              <div
                key={`${it.x},${it.y},${i}`}
                className="item"
                data-src={(it as any).src}
                data-offset-x={(it as any).x}
                data-offset-y={(it as any).y}
                data-size-x={(it as any).sizeX}
                data-size-y={(it as any).sizeY}
                style={{
                  ["--offset-x" as any]: (it as any).x,
                  ["--offset-y" as any]: (it as any).y,
                  ["--item-size-x" as any]: (it as any).sizeX,
                  ["--item-size-y" as any]: (it as any).sizeY,
                }}
              >
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={(it as any).alt || "Open image"}
                  onClick={onTileClick}
                  onPointerUp={onTilePointerUp}
                >
                  <img src={(it as any).src} draggable={false} alt={(it as any).alt} />
                </div>
              </div>
            ))}
          </div>
